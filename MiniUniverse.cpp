...
const TColor HZ37_DeselectedEvenTrack = 0x00999999;

// Deselected region on odd track.

const TColor HZ37_DeselectedOddTrack = 0x00888888;
...
)
{
	...
	{
		YCoordLookupTable[Tracks[Idx].TrackName] = FHeight - ((TrackCount - Idx) * H) - 1;
	...
	for(std::vector <TRegion>::iterator Region = Regions.begin(); Region != Regions.end(); ++Region)
	{
		...
		FDeselectedRenderBuffer->Canvas->Pen->Color = (Odd ? HZ37_DeselectedOddTrack : HZ37_DeselectedEvenTrack);
